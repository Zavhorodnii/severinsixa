...
class Mo_OAuth_Client_Admin_Guides {
	...
	public static function instructions_page($appname){
		...
		if($app->guide !== "") {
			echo '<li>Please follow the instruction given in Step by Step guide to get your Client ID, Client Secret and other necessary information. <a href="'.esc_attr($app->guide).'" target="_blank">Click here</a> to get the step by step guide to configure <i>'.explode('App', esc_attr($app->label))[0].'</i> as OAuth/OpenID Connect server.</li>';
		} else {
			echo '
			...
			<li>Choose the scopes as per your application/OAuth Server specification(if provided) and enter the same on the <a href="admin.php?page=mo_oauth_settings&appId='.esc_attr($app->appId).'" target="_blank"><b>'.MO_OAUTH_ADMIN_MENU.' -> Configure OAuth</b></a> page.</li>
