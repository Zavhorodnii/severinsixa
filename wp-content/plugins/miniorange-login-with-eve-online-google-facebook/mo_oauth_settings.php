...
define( 'MO_OAUTH_SETUP_WIZARD_VERSION', '2.0.0' );
define( 'MO_OAUTH_CSS_JS_VERSION', '6.22.2' );
class mo_oauth {
...
//                    }
		...
		elseif( isset( $_POST['option'] ) and sanitize_text_field( wp_unslash( $_POST['option'] ) ) == "mo_oauth_client_demo_request_form" && isset($_REQUEST['mo_oauth_client_demo_request_field']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['mo_oauth_client_demo_request_field'])), 'mo_oauth_client_demo_request_form') ) {
			...
			$email = get_option("mo_oauth_admin_email")?get_option("mo_oauth_admin_email"):"";
			$feedback_reasons = new Mo_OAuth_Client_Customer();
			$feedback_reasons->mo_oauth_send_email_alert( $email, '', '', "Feedback: WordPress ".MO_OAUTH_PLUGIN_NAME , true );
			deactivate_plugins( __FILE__ );
			update_option( 'message', 'Plugin deactivated successfully' );
			$this->mo_oauth_show_success_message();
		}
		// else if ( isset( $_POST['mo_oauth_client_feedback'] ) and sanitize_text_field( wp_unslash( $_POST['mo_oauth_client_feedback'] ) ) == 'true' && isset( $_REQUEST['mo_oauth_feedback_form_field'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['mo_oauth_feedback_form_field'] ) ), 'mo_oauth_feedback_form') && isset($_POST['miniorange_feedback_skip']) && sanitize_text_field( wp_unslash( $_POST['miniorange_feedback_skip'] ) ) == 'Skip') {
			...
			if( current_user_can( 'administrator' ) ) {
				...
				if ( $deactivate_reason ) {
					...
					if(isset($_POST['miniorange_feedback_skip']) && sanitize_text_field( wp_unslash( $_POST['miniorange_feedback_skip'] ) ) == 'Skip'){
						$feedback_reasons->mo_oauth_send_email_alert( $email, $reply, $message, " Skip Feedback: WordPress ".MO_OAUTH_PLUGIN_NAME , true );
						deactivate_plugins( __FILE__ );
						if( !array_key_exists( 'mo_oauth_keep_settings_intact', $_POST ) ){
							$this->delete_options_on_deactivation();
						}
